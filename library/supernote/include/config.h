...
#define ERASALINE_FIVESTAR_SIGNAL         5  //精细擦 擦除笔迹 擦除五角星 未擦除标题或链接
#define ERASAAREA_SHADOWANDTRAIL_SIGNAL   6  //区域擦 擦除标题和笔迹  刷新页面和标题
#define ERASALINE_SHADOW_SIGNAL           7  //精细擦 擦除标题,未擦除五角星 只刷新标题
#define ERASALINE_FIVESTAR_SHADOW_SIGNAL  8  //精细擦 擦除五角星 擦除标题 刷新标题和页面
...
enum SNFILE_RETURN_CODE {
    ...
    SNFILE_FETCH_MODULE_FEATURE_FAILED = 13,          //fetch data of module feature failed
    SNFILE_FETCH_MODULE_INDEX_FAILED = 14,            //fetch data of module index failed
    SNFILE_MODULE_TYPE_ERROR = 15,                    //file type of module page is difference of module feature
    ...
    SNFILE_FETCH_MODULE_PAGE_FEATURE_FAILED = 26,     //fetch data of module page feature failed
    SNFILE_FETCH_MODULE_LAYER_FEATURE_FAILED = 27,    // etch data of module layer feature failed
    ...
    SNFILE_FILEVERSION_HIGHER_THAN_APPVERSION = 50,   //old file version, need update
    ...
    SNFILE_A5A6FILE_VERSION_ERROR = 57,               //A5A6 file format is not the latest version,need update first.
};

//gray eink color should only be 16 kinds, more is in drawlinelive.cpp using fake color below
enum COLOR_INDEX{
...
const unsigned char COMPRESS_INDEX[] = {
        0x61, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,//0-15
        ...
        0x63, 0x67, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,//48-63
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,//64-79
        0x64, 0x68, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,//80-95
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x62//240-255
};
const unsigned int UNCOMPRESS_INDEX[] = {
        ...
        0xFF, 0x00, 0xFF, 0x30, 0x50, 0xFE, 0x01, 0x31, 0x51, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,//96-111
...
struct PageFeatureReadFlag {
    ...
    void setValues(bool p_pageStyleFlag = false,
                   bool p_pageStyleMd5 = false,
                   bool p_mainLayerFlag = false,
                   bool p_layer1Flag = false,
                   bool p_layer2Flag = false,
                   bool p_layer3Flag = false,
                   bool p_bglayerFlag = false,
                   bool p_totalPathFlag = false,
                   bool p_trailsSizeFlag = false,
                   bool p_trailsParseFlag = false,
                   bool p_fiveStarFlag = false,
                   bool p_layerInfoFlag = false,
                   bool p_layerSeqFlag = false,
                   bool p_thumbnailTypeFlag = false,
                   bool p_recognStatusFlag = false,
                   bool p_recognTextFlag = false,
                   bool p_recognFileFlag = false,
                   bool p_pageIdFlag = false,
...
struct fiveStarDB {
    ...
    int x5 = 0;
    int y5 = 0;
    int instFlag = 1; //insert Flag, if 1,need add record in file, if 0, need delete record from file.
};
...
extern bool flushfsyncfclose(FILE *&fp, bool flushFlag, bool fsyncFlag, bool fcloseFlag);
extern bool uncompresseDataByRLE(unsigned char *destData, size_t destDataLen, unsigned char *srcData, size_t srcDataLen);
extern int unitForRLE(int &ValueNum, unsigned char Value, unsigned char *origData, size_t &destDataLen);
extern bool compresseDataByRLE(unsigned char data[], size_t dataSize, unsigned char *destData, size_t &destDataLen);
extern vector<string> split(std::string str, std::string pattern);
extern int data2Trail(ratta_cache_type *buff, int size, TrailContainer &trailContainer);
extern bool readModuleItemFromFile(FILE *&fp, vector<ModuleItem> &paraCite, size_t nSite);

extern bool mappingTrailPoints2imageSize(const TrailContainer &trailContainer, int imageWidth, int imageHeight, std::vector<RattaPoint> &pointsOut, int mapVersion=0);
